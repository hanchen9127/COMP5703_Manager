...
import {
  useProjectRouteBreadcrumbs,
  type ProjectRouteBreadcrumbLabels,
} from "@/hooks/use-project-route-breadcrumbs"
import {
  useTaskRouteBreadcrumbs,
  type TaskRouteBreadcrumbLabels,
} from "@/hooks/use-task-route-breadcrumbs"
import { parseProjectIdFromBackendTaskId } from "@/lib/live-hydrate"

function buildProjectWorkspaceBreadcrumbs(
  projectId: string,
  pathname: string,
  labels: Record<string, string>,
  projectRouteLabels: ProjectRouteBreadcrumbLabels | null,
) {
  const projectLabel = projectRouteLabels?.projectName ?? projectId
  const breadcrumbSegments = pathname.split("/").filter(Boolean)
  const projectSubpage = breadcrumbSegments[2]

  const crumbs = [
    { href: "/projects", label: "Projects" },
    { href: `/projects/${projectId}`, label: projectLabel },
  ]

  if (projectSubpage) {
    crumbs.push({
      href: pathname,
      label: labels[projectSubpage] ?? projectSubpage,
    })
  }
...
function buildTaskWorkspaceBreadcrumbs(
  taskId: string,
  pathname: string,
  labels: Record<string, string>,
  taskRouteLabels: TaskRouteBreadcrumbLabels | null,
) {
  const projectId =
    taskRouteLabels?.projectId ?? parseProjectIdFromBackendTaskId(taskId)
  if (!projectId) {
    return null
  }

  const projectLabel = taskRouteLabels?.projectName ?? projectId
  const taskLabel = taskRouteLabels?.taskTitle ?? taskId
  const breadcrumbSegments = pathname.split("/").filter(Boolean)
  ...
  const crumbs = [
    ...
    { href: `/tasks/${taskId}`, label: taskLabel },
  ]
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const projectIdFromPath =
    breadcrumbSegments[0] === "projects" &&
    breadcrumbSegments[1] &&
    breadcrumbSegments[1] !== "new"
      ? breadcrumbSegments[1]
      : null
  const taskIdFromPath =
    breadcrumbSegments[0] === "tasks" && breadcrumbSegments[1]
      ? breadcrumbSegments[1]
      : null
  const projectRouteLabels = useProjectRouteBreadcrumbs(projectIdFromPath)
  const taskRouteLabels = useTaskRouteBreadcrumbs(taskIdFromPath)
  const navItems = [
    {
      href: "/",
      label: "Dashboard",
    ...
    breadcrumbSegments.length === 0
      ...
      : (() => {
          ...
          if (
            ...
            const projectId = breadcrumbSegments[1]

            return buildProjectWorkspaceBreadcrumbs(
              projectId,
              pathname,
              labels,
              projectRouteLabels,
          ...
          if (breadcrumbSegments[0] === "tasks" && breadcrumbSegments[1]) {
            const taskId = breadcrumbSegments[1]
            const taskWorkspaceCrumbs = buildTaskWorkspaceBreadcrumbs(
              taskId,
              pathname,
              labels,
              taskRouteLabels,
            )
            if (taskWorkspaceCrumbs) {
              return taskWorkspaceCrumbs
            }
          }
        ...
        <aside className="rounded-[1.35rem] border border-slate-900/12 bg-[linear-gradient(180deg,rgba(255,252,247,0.96),rgba(247,239,223,0.99))] p-4 shadow-[0_12px_30px_rgba(15,23,42,0.05)] dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.94),rgba(17,24,39,0.98))] dark:shadow-[0_18px_40px_rgba(2,6,23,0.35)]">
          ...
          <div className="hej-surface-soft mt-6 rounded-[1rem] border border-slate-900/10 bg-white/88 p-3.5 dark:border-white/10">
            ...
            <p className="mt-2 text-[13px] leading-5 text-slate-600 dark:text-slate-300">
              Project → Task → Items → Annotate → Review → Dispute → Export
            </p>
            <Button asChild className="mt-3 w-full bg-amber-300 text-slate-900 hover:bg-amber-200">
              <Link href="/projects">Start workflow</Link>
            </Button>
          </div>
        </aside>
