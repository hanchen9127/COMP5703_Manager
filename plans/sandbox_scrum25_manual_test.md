...
```powershell
cd D:\COMP5703_Capstone\docs\plans
python sandbox_scrum25.py new
```
...
```powershell
python sandbox_scrum25.py check <item>
```
...
> 这一步改了数据库。做完直接 `python sandbox_scrum25.py new` 换个干净条目，别把改过的数据留着。
...
```powershell
python sandbox_scrum25.py clean          # 只列出将要删除的
python sandbox_scrum25.py clean --yes    # 真的删除
```
