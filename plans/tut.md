...
Companion: [`plan.md`](./plan.md) — the commit-by-commit plan for SCRUM-25 / 26 / 28.
...
| `sandbox_text_003` | `item_0d3bb2fc6864` | canonicalized · **3 drafts** · **2 annotations**, both `is_latest=1` / `version=1` | was "kept clean" — now the D4 reproduction (see `plan.md`, *Known boundary*) |
...
One trap that will bite you if you skim, spelled out in `plan.md`: `verify_user_draft_access` is
...
`docs/design/backend/api_surfaces.md`. That's `plan.md`'s Commit 6, and it's not optional politeness
...
| `docs/issues.md` | 29 verified defects, prioritized | Verified against code |
| `docs/fix-plan.md` | Per-defect fix recipes with line refs | Yours: sections 2, 6, 11. **§6 item 1 is wrong** — see below |
| `docs/current_user_stories.md` | 59 stories, acceptance criteria, DoD | The source for "am I done?" |
| `docs/Jira.csv` | Board export — ticket ↔ story ↔ issue | Sprint assignment |
...
- **`fix-plan.md` §6 item 1 is wrong.** It says to put the ownership check inside
  ...
  following it would 403 every reviewer approval. `plan.md` deviates deliberately — say so in your PR.
...
- Two live decisions are **blocked on the client**, and one touches you: D5 criterion 2 (what
  ...
  invent. `plan.md` scopes SCRUM-28 to refusal only for that reason.

### If something looks broken, check in this order

1. `docs/issues.md` — is it one of the 29?
...
2. Re-create the extra accounts: `.venv\Scripts\python ..\..\..\docs\plans\seed_test_roles.py`.
...
7. Open [`plan.md`](./plan.md) and start at Commit 1.
