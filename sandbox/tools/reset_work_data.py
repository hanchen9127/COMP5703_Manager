...
def _find_db() -> Path:
    """Locate hej_dev.db by walking up to the folder that contains the hej repo.

    A fixed parents[N] breaks whenever this script moves; searching upward does not.
    """
    for parent in Path(__file__).resolve().parents:
        api_dir = parent / "hej" / "apps" / "hej-api"
        if api_dir.is_dir():
            return api_dir / "hej_dev.db"
    raise SystemExit("Could not find hej/apps/hej-api above this script.")
...
def main() -> None:
    ...
    try:
        ...
        print("\nNext: python docs/sandbox/W6/scripts/sandbox-SCRUM-25.py new")
    finally:
        conn.close()
