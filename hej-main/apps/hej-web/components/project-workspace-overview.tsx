...
import type { MockExportPackage, MockProject, MockTask, MockDisputeCase } from "@/lib/mock-data"
...
export function ProjectWorkspaceOverview({
  ...
  const judgementTasks = tasks.filter((task) => task.taskClass === "judgement")
  const primaryTask = draftTasks[0] ?? activeTasks[0] ?? tasks[0] ?? null
  const portfolioSnapshot = tasks.slice(0, 3)

  const executionSurfaces = primaryTask
    ? [
        {
          title: "Task setup",
    ...
    <div className="space-y-4">
      <section className="grid gap-3 md:grid-cols-4">
        {[
          ...
          { label: "Judgement tasks", value: judgementTasks.length.toString() },
        ...
        <Card className="hej-surface-dark rounded-[1.3rem] border-slate-900/10 bg-white/80 shadow-[0_12px_30px_rgba(15,23,42,0.05)] dark:border-white/10">
          <CardHeader className="px-4 md:px-5">
            ...
            <CardDescription className="mt-1 max-w-2xl text-[13px] leading-5 text-slate-600">
              Project overview should immediately show which tasks exist and where execution pressure currently sits.
            </CardDescription>
          </CardHeader>
              ...
              <Link
                ...
                className={
                  primaryTask?.id === task.id
                    ? "block rounded-xl border border-amber-300/70 bg-[linear-gradient(180deg,rgba(255,251,235,0.96),rgba(250,244,230,0.92))] p-3.5 shadow-[0_12px_24px_rgba(217,119,6,0.10)] transition-all duration-200 hover:-translate-y-0.5 hover:border-amber-400 hover:shadow-[0_16px_28px_rgba(217,119,6,0.14)]"
                    : "hej-surface-soft block rounded-xl border border-slate-900/10 bg-stone-50/88 p-3.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-900/20 hover:bg-white hover:shadow-[0_12px_24px_rgba(15,23,42,0.08)] dark:border-white/10 dark:hover:border-white/20 dark:hover:bg-white/10"
                }
              >
        ...
        <Card className="hej-surface-dark rounded-[1.3rem] border-slate-900/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.84),rgba(245,235,214,0.9))] shadow-[0_12px_30px_rgba(15,23,42,0.05)] dark:border-white/10">
          ...
          <CardContent className="space-y-3 px-4 md:px-5">
            {[
              ...
              {
                label: "Exports",
                detail: `${exports.length} delivery paths track final outputs and provenance packaging.`,
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
          ...
          <CardContent className="grid gap-3 px-4 md:px-5 sm:grid-cols-2">
            {executionSurfaces.map(({ title, text, href, icon: Icon }) => (
              <Link
                key={title}
                href={href}
                className="hej-surface-soft rounded-xl border border-slate-900/10 bg-stone-50/88 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-900/20 hover:bg-white hover:shadow-[0_12px_24px_rgba(15,23,42,0.08)] dark:border-white/10 dark:hover:border-white/20 dark:hover:bg-white/10"
              >
        ...
        <Card className="hej-surface-dark rounded-[1.3rem] border-slate-900/10 bg-white/80 shadow-[0_12px_30px_rgba(15,23,42,0.05)] dark:border-white/10">
          <CardHeader className="px-4 md:px-5">
            ...
            <CardDescription className="mt-1 max-w-2xl text-[13px] leading-5 text-slate-600">
              Project overview should keep task execution primary while preserving cross-task governance visibility.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 px-4 md:px-5">
            {[
